            ...
            if (!empty($row['username'])){
                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if (!$row = mysqli_fetch_assoc($resultl)){
                    	...
                    	$sql = "INSERT INTO users_logs (username, class, section, serialnumber, fingerprint_id, checkindate, timein, timeout) VALUES (? ,?, ?, CURDATE(), CURTIME(), ?)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
                        	...
                        	$timeout = "";
                            mysqli_stmt_bind_param($result, "sids", $Uname, $class, $section, $Number, $fingerID, $timeout);
                            mysqli_stmt_execute($result);
            ...
            if (!mysqli_stmt_prepare($result, $sql)) {
                ...
                if ($row = mysqli_fetch_assoc($resultl)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $sql = "INSERT INTO users (username , class, section, serialnumber, gender, fingerprint_id, fingerprint_select) VALUES (?, ?, ?, ?, ?, ?, ?)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            mysqli_stmt_bind_param($result, "sids", $Uname, $Number, $gender, $fingerID);
                            mysqli_stmt_execute($result);
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        mysqli_stmt_bind_param($result, "sids", $Uname, $class, $section, $Number, $gender, $fingerID);
                        mysqli_stmt_execute($result);
