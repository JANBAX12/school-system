      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if (empty($row['username'])) {
                ...
                if (!empty($Uname) && !empty($class) && !empty($section) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql="UPDATE users SET username=?, class=?, section=?, serialnumber=?, gender=?, email=?, user_date=CURDATE(), time_in=?, time_out=?, WHERE fingerprint_select=1";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
                                ...
                                mysqli_stmt_bind_param($result, "sdsss", $Uname, $class, $section, $Number, $Gender, $Email, $Timein, $timeout );
                                mysqli_stmt_execute($result);
      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if (empty($row['username'])) {
                ...
                if (empty($Uname) && empty($class) && empty($section) && empty($Number) && empty($Email) && empty($Timein) && empty($timeout)) {
                    echo "Empty Fields";
                    exit();
                }
                else{
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($class) && !empty($section) && !empty($Email) && !empty($Timein) && !empty($timeout)) {

                                $sql="UPDATE users SET username=?, class=?, section=?, serialnumber=?, gender=?, email=?, time_in=?, time_out=?, WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
                                    ...
                                    mysqli_stmt_bind_param($result, "sids", $Uname, $class, $section, $Number, $Gender, $Email, $Timein );
                                    mysqli_stmt_execute($result);
                                ...
                                if (!empty($timeout)){
                                    $sql="UPDATE users SET gender=?, time_out=? WHERE fingerprint_select=1";
                                    $result = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result, $sql)) {
                                        echo "SQL_Error_select_Fingerprint";
                                        exit();
...
if (isset($_POST['delete'])) {
    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        $resultl = mysqli_stmt_get_result($result);
        if ($row = mysqli_fetch_assoc($resultl)) {
            $sql="UPDATE users SET username='', class='', section='', serialnumber='', gender='', email='', time_in='', del_fingerid=1 WHERE fingerprint_select=1";
            $result = mysqli_stmt_init($conn);
            if (!mysqli_stmt_prepare($result, $sql)) {
